...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="MagicLamp.py GasGenie",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=textwrap.dedent('''
        *******************************************************

        GasGenie - part of the MagicLamp suite.
  ...
  \    )\.-.      /`-.     )\.--.     )\.-.    )\.---.   )\  )\  .'(   )\.---.  
    ...
    parser.add_argument('-out', type=str, help="name output directory (default=gasgenie_out)",
    ...
    parser.add_argument('--norm', type=str,
                        help="include this flag if you would like the gene counts for each iron gene category to be normalized to "
                             ...
                             "normalization, GasGenie will create a heatmap-compatible "
                             "CSV output with raw gene counts. With normalization, GasGenie will create a "
                             "heatmap-compatible with \'normalized gene abundances\'", const=True, nargs="?")

    parser.add_argument('--makeplots', type=str,
                        help="include this flag if you would like GasGenie to make some figures from your data?. "
                             ...
                             "see the main output (CSV files) from GasGenie.", const=True, nargs="?")

    # CHECKING FOR CONDA INSTALL
    os.system("echo ${gas_hmms} > HMMlib.txt")
    ...
    if args.bin_ext != "NA":
        ...
        print(
            'Looks like you did not provide an extension for your genomes/bins or assemblies, so GasGenie does not know'
    ...
    try:
        ...
        # answer = input("Would you like GasGenie to proceed and potentially overwrite files in this directory? (y/n): ")
        answer = "y"
        if answer == "y":
                    ...
                    for line in testFile:
                        if re.match(r'>', line):
                            if re.findall(r'\|]', line):
                                ...
                                print(
                                    "Unfortunately, this is a problem for GasGenie because it uses that character as delimiter to store important information.")
    ...
    deFuckDict = defaultdict(lambda: defaultdict(lambda: 'EMPTY'))
    meta = open(bits, "r")
    metaDict = defaultdict(lambda: defaultdict(lambda: 'EMPTY'))
    for i in meta:
        ls = i.rstrip().split("\t")
        metaDict[ls[0]] = ls[1]
        deFuckDict[ls[0]] = ls[2]
    ...
    for i in binDirLS:  # ITERATION THROUGH EACH BIN IN A GIVEN DIRECTORY OF BINS
        if lastItem(i.split(".")) == args.bin_ext:  # FILTERING OUT ANY NON-BIN-RELATED FILES
            ...
            for hmm in HMMdirLS:  # ITERATING THROUGH ALL THE HMM FILES IN THE HMM DIRECTORY
                ...
                sys.stdout.flush()
                if len(metaDict[hmm.split(".hm")[0]]) == 0:

                    bit = 0
                else:
                    bit = metaDict[hmm.split(".hm")[0]]
    ...
    for i in summary:
        ls = i.rstrip().split(",")
        if ls[0] != "category" and ls[0] != "GasGenie":
            if len(ls) > 0:
                category = ls[0]
                cell = ls[0]
                orf = ls[1]
                ...
                evalue = ls[3]
                hmmBit = ls[4]
                bitcut = metaDict[hmm.split(".hm")[0]]
...
# ****************************** FILTERING OUT POTENTIAL FALSE POSITIVES *************************************
    ...
    for i in summary:
        if not re.match(r'#', i):
            ls = i.rstrip().split(",")
            clu = int(ls[6])
            gene = deFuckDict[ls[2].split(".hm")[0]]
            clusterDict[clu]["line"].append(ls)
            clusterDict[clu]["gene"].append(gene)
        ...
        if "gvpA" in ls or "gvpB" in ls or "gvpC" in ls or "gvpD" in ls or "gvpE" in ls or "gvpF" in ls or "gvpG" in ls or "gvpH" \
            ...
            if unique(ls, gvp) < 1:
                ...
                for j in clusterDict[i]["line"]:
                    gene = (deFuckDict[j[2].split(".hm")[0]])
                    out.write(j[0] + "," + j[1] + "," + gene + "," + j[3] + "," + j[4] + "," + j[5] + "," + j[6] + "," + j[7] + "\n")
    ...
    os.system("mv %s/summary-3.csv %s/gasgenie-summary.csv" % (args.out, args.out))
    ...
    if args.bams != "NA":
        ...
        final = open("%s/gasgenie-summary.csv" % (args.out), "r")
        for i in final:
            ls = (i.rstrip().split(","))
        ...
        outHeat = open("%s/gasgenie.readDepth.heatmap.csv" % (args.out), "w")
        outHeat.write("X" + ',')
    ...
    # GENE COUNTS-BASED ABUNDANCE
        ...
        outHeat = open("%s/gasgenie.heatmap.csv" % (outDirectory), "w")
        outHeat.write("X" + ',')
        for i in sorted(Dict.keys()):
            outHeat.write(i + ",")
        outHeat.write("\n")
    ...
    if args.makeplots:
        ...
        if args.norm:
            os.system("Rscript --vanilla %s/DotPlot.R %s/gasgenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/gasgenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
        else:
            os.system("Rscript --vanilla %s/DotPlot-nonorm.R %s/gasgenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/gasgenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
    ...
    if args.makeplots:
        ...
        if args.bam == "NA" and args.bams == "NA":
            ...
            os.system("Rscript --vanilla %s/DotPlot.R %s/gasgenie.readDepth.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/gasgenie.readDepth.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))

    print("")
    print("Results are written to %s/gasgenie-summary.csv and %s/gasgenie.heatmap.csv" % (args.out, args.out))
