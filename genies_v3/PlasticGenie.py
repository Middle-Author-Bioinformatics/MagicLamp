...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="MagicLamp.py PlasticGenie",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=textwrap.dedent('''
        *******************************************************

        PlasticGenie - part of the MagicLamp suite.
    ...
    parser.add_argument('-out', type=str, help="name output directory (default=plasticgenie_out)",
                        default="plasticgenie_out")
    ...
    def have_hmm_lib(root):
        if not root or not os.path.isdir(root):
            return False
        return any(f.lower().endswith(".hmm") for f in os.listdir(root))

    if not have_hmm_lib(HMMdir):
    ...
    try:
        ...
        # answer = input("Would you like PlasticGenie to proceed and potentially overwrite files in this directory? (y/n): ")
        answer = "y"
        if answer == "y":
