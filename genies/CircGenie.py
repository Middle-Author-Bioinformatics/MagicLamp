...
def main():
    ...
    parser.add_argument('-out', type=str, help="name output directory (default=circgenie_out)",
                        default="circgenie_out")
    ...
    parser.add_argument('--norm', type=str,
                        help="include this flag if you would like the gene counts for each iron gene category to be normalized to "
                             ...
                             "normalization, CircGenie will create a heatmap-compatible "
                             "CSV output with raw gene counts. With normalization, CircGenie will create a "
                             "heatmap-compatible with \'normalized gene abundances\'", const=True, nargs="?")

    parser.add_argument('--makeplots', type=str,
                        help="include this flag if you would like CircGenie to make some figures from your data?. "
                             ...
                             "Please be sure to install all the required R packages as instrcuted in the CircGenie Wiki: "
                             "https://github.com/Arkadiy-Garber/CircGenie/wiki/Installation. "
                             "If you see error or warning messages associated with Rscript, you can still expect to "
                             "see the main output (CSV files) from CircGenie.", const=True, nargs="?")

    # CHECKING FOR CONDA INSTALL
    os.system("echo ${circ_hmms} > HMMlib.txt")
    ...
    if args.bin_ext != "NA":
        ...
        print(
            'Looks like you did not provide an extension for your genomes/bins or assemblies, so CircGenie does not know'
    ...
    try:
        ...
        answer = input("Would you like CircGenie to proceed and potentially overwrite files in this directory? (y/n): ")
        if answer == "y":
                    ...
                    for line in testFile:
                        if re.match(r'>', line):
                            if re.findall(r'\|]', line):
                                ...
                                print(
                                    "Unfortunately, this is a problem for CircGenie because it uses that character as delimiter to store important information.")
                ...
                if lastItem(hmm.split(".")) == "hmm":
                    ...
                    if args.orfs:
                        ...
                        os.system(
                            ...
                            % (int(args.t), float(bit), outDirectory, i, hmm, outDirectory, i, hmm, HMMdir, hmm,
                               outDirectory, i)
                        )
                        ...
                        if not re.match(r'#', line):
                            ...
                            if evalue < float(1E-1):
                                ...
                                if orf not in HMMdict[i]:
                                    ...
                                    if hmm.split(".hm")[0] in ["CikA_GAF", "CikA_HATPase_c", "CikA_HisKA"] and \
                                                    HMMdict[i][orf]["hmm"] in ["CikA_GAF", "CikA_HATPase_c", "CikA_HisKA",
                                                                               "CikA_HisKA ; CikA_HATPase_c",
                                                                               "CikA_HisKA ; CikA_GAF",
                                                                               "CikA_GAF ; CikA_HisKA",
                                                                               "CikA_GAF ; CikA_HATPase_c",
                                                                               "CikA_HATPase_c ; CikA_GAF",
                                                                               "CikA_HATPase_c ; CikA_HisKA"]:

                                        newHMM = HMMdict[i][orf]["hmm"] + " ; " + hmm.split(".hm")[0]
                                        HMMdict[i][orf]["hmm"] = newHMM
                                        HMMdict[i][orf]["evalue"] = evalue
                                        HMMdict[i][orf]["bit"] = bit
                                    else:
    ...
    for i in summary:
        ls = i.rstrip().split(",")
        if ls[0] != "category" and ls[0] != "CircGenie":
            if len(ls) > 0:
                category = ls[0]
                cell = ls[0]
                orf = ls[1]
                ...
                evalue = ls[3]
                hmmBit = ls[4]
                if hmm.split(" ; ")[0] == "CikA_HisKA" or hmm.split(" ; ")[0] == "CikA_GAF" or hmm.split(" ; ")[0] == "CikA_HATPase_c":
                    if len(hmm.split(" ; ")) == 3:
                        bitcut = metaDict[hmm]
                ...
                if len(RemoveDuplicates(k)) == 1:
                    ...
                    out.write(
                        i + "," + orf + "," + SummaryDict[i][orf]["hmm"] + "," + SummaryDict[i][orf]["e"] + "," + str(
                            SummaryDict[i][orf]["hmmBit"]) + "," + str(SummaryDict[i][orf]["bitcut"]) + "," + str(
                            counter) + "," + str(SummaryDict[i][orf]["seq"]) + "\n")
                    out.write(
                        "#" + "," + "#" + "," + "#" + "," + "#" + "," + "#" + "," + "#" + "," + "#" + "," + "#" + "\n")
                    counter += 1
    ...
    if args.bams != "NA":
        ...
        outHeat = open("%s/circgenie.readDepth.heatmap.csv" % (args.out), "w")
        outHeat.write("X" + ',')
    ...
    # GENE COUNTS-BASED ABUNDANCE
        ...
        outHeat = open("%s/circgenie.heatmap.csv" % (outDirectory), "w")
        outHeat.write("X" + ',')
        for i in sorted(Dict.keys()):
            outHeat.write(i + ",")
        outHeat.write("\n")
    ...
    if args.makeplots:
        print("Running Rscript to generate plots. Do not be alarmed if you see Warning or Error messages from Rscript. "
              ...
              "If not, you can plot the data as you wish on your own, or start an issue on CircGenie's GitHub repository\n")
        # if conda == 0:
        #     Rdir = args.R
        # else:
        ...
        if args.norm:
            os.system(
                "Rscript --vanilla %s/DotPlot.R %s/circgenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/circgenie.heatmap.csv %s/" % (
            rscriptDir, outDirectory, outDirectory))
        else:
            os.system("Rscript --vanilla %s/DotPlot-nonorm.R %s/circgenie.heatmap.csv %s/" % (
            rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/circgenie.heatmap.csv %s/" % (
            rscriptDir, outDirectory, outDirectory))
    ...
    if args.makeplots:
        ...
        if args.bam == "NA" and args.bams == "NA":
            if args.norm:
                os.system("Rscript --vanilla %s/DotPlot.R %s/circgenie.heatmap.csv %s/" % (
                rscriptDir, outDirectory, outDirectory))
                os.system("Rscript --vanilla %s/dendro-heatmap.R %s/circgenie.heatmap.csv %s/" % (
                rscriptDir, outDirectory, outDirectory))
            else:
            ...
            os.system("Rscript --vanilla %s/DotPlot.R %s/circgenie.readDepth.heatmap.csv %s/" % (
            rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/circgenie.readDepth.heatmap.csv %s/" % (
            rscriptDir, outDirectory, outDirectory))

    print("")
    print("Results are written to %s/circgenie-summary.csv and %s/circgenie.heatmap.csv" % (args.out, args.out))
