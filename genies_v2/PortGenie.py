...
# PortNaGenie: HMM-based identification and categorization of sodium-coupled
# membrane transporters (Na+/H+ antiporters and Na+/solute symporters).
#
# Part of the MagicLamp suite (https://github.com/Arkadiy-Garber/MagicLamp).
# Modeled on Lucifer.py and ATPGenie.py for stylistic consistency.
#
# All TIGRFAM HMMs are run with hmmsearch --cut_tc, using the trusted
# cutoffs built into each HMM. Each HMM is hard-coded to one of four
# transport mode categories (plus an "other" bucket for the two HMMs in
# the input set that are NOT canonical Na+ transporters).


def main():

    # =================================================================
    # Helper functions (mirrored from Lucifer.py / ATPGenie.py)
    # =================================================================
    ...
    parser = argparse.ArgumentParser(
        prog="MagicLamp.py PortNaGenie",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=textwrap.dedent(r'''
        *******************************************************

        PortNaGenie - part of the MagicLamp suite.
        Developed by Arkadiy Garber, Daniel Naphas, and collaborators.
        Please send comments and inquiries to ark@midauthorbio.com

          ____           _   _   _        ____            _
         |  _ \ ___  _ _| |_| \ | | __ _ / ___| ___ _ __ (_) ___
         | |_) / _ \| '_| __|  \| |/ _` | |  _ / _ \ '_ \| |/ _ \
         |  __/ (_) | | | |_| |\  | (_| | |_| |  __/ | | | |  __/
         |_|   \___/|_|  \__|_| \_|\__,_|\____|\___|_| |_|_|\___|

                  Sodium-coupled transporter profiler
            Na+/H+ antiporters and Na+/solute symporters

        Single-pass analysis:
          Profiles genomes/proteomes with curated TIGRFAM HMMs for
          sodium-coupled membrane transporters and classifies each
          hit as one of:
              - Na+:H+ antiport
              - Na+:H+ symport            (none known biologically)
              - Na+:solute antiport       (none known biologically)
              - Na+:solute symport
              - other                     (non-Na or primary ATPase)
    ...
    parser.add_argument('-d', type=int, help="maximum distance between genes to be considered in a genomic 'cluster'."
                                             ...
                                             "of genes in between putative sodium-transporter-related genes identified "
                                             "by the HMM database (default=5)", default=5)

    parser.add_argument('-out', type=str, help="name output directory (default=portnagenie_out)",
                        default="portnagenie_out")
    ...
    parser.add_argument('--norm', type=str,
                        help="include this flag if you would like the gene counts for each transporter category to "
    ...
    os.system("echo ${portna_hmms} > HMMlib.txt")
    ...
    if not have_lib(HMMrootdir):
        ...
        try:
            ...
            HMMrootdir = location + "/hmms/portna_hmms"
        except FileNotFoundError:
            pass

    if not have_lib(HMMrootdir):
        print("PortNaGenie could not locate its HMM library. Please run the setup.sh script if\n"
              "you have Conda installed. Otherwise, please run setup-noconda.sh and put MagicLamp.py\n"
              "into your $PATH. The library must contain a 'sodium-transporters/' subdirectory.")
    ...
    # =================================================================
    # HMM metadata
    #   acc -> (transport_mode, gene_symbol, description)
    #
    # Transport modes:
    #   "Na:H_antiport"      Na+:H+ antiporter
    #   "Na:H_symport"       Na+:H+ symporter (no canonical examples)
    #   "Na:solute_antiport" Na+:solute antiporter (no canonical examples)
    #   "Na:solute_symport"  Na+:solute symporter
    #   "other"              not a sodium transporter at all (TIGR00900 = drug:H+ efflux,
    #                        TIGR01106 = P-type ATPase = primary pump, not secondary
    #                        transporter); kept for reference but flagged
    # =================================================================
    HMM_META = {
        # Na+/H+ antiporters
        "TIGR00773": ("Na:H_antiport",      "NhaA",         "Na+/H+ antiporter NhaA"),
        "TIGR00774": ("Na:H_antiport",      "NhaB",         "Na+/H+ antiporter NhaB"),
        "TIGR00775": ("Na:H_antiport",      "NhaD",         "NhaD family Na+/H+ antiporter"),
        "TIGR00831": ("Na:H_antiport",      "CPA1_archaeal","CPA1 family Na+/H+ antiporter (archaeal)"),
        "TIGR00844": ("Na:H_antiport",      "CPA1_cyano",   "CPA1 family Na+/H+ antiporter (cyanobacterial)"),
        "TIGR00931": ("Na:H_antiport",      "NhaC",         "Na+/H+ antiporter NhaC"),
        "TIGR00932": ("Na:H_antiport",      "CPA2",         "CPA2 monovalent cation:H+ antiporter (often K+/H+ KefB/C)"),
        # Mrp/Pha/Mnh multi-subunit Na+/H+ antiporter complex
        "TIGR00941": ("Na:H_antiport",      "MnhC_MrpC",    "Mrp Na+/H+ antiporter subunit C"),
        "TIGR00942": ("Na:H_antiport",      "MnhE_MrpE",    "Mrp Na+/H+ antiporter subunit E"),
        "TIGR00943": ("Na:H_antiport",      "MnhB_MrpB",    "Mrp cation/H+ antiporter subunit B"),
        "TIGR00944": ("Na:H_antiport",      "MrpD_PhaD",    "Mrp K+/H+ antiporter subunit D (part of Na+/H+ Mrp complex in vivo)"),
        "TIGR01300": ("Na:H_antiport",      "MnhG_PhaG",    "Mrp cation/H+ antiporter subunit G"),
        # Na+/solute symporters
        "TIGR00210": ("Na:solute_symport",  "gltS",         "Sodium/glutamate symporter"),
        "TIGR00785": ("Na:solute_symport",  "DASS",         "DASS family sodium-coupled anion symporter"),
        "TIGR00813": ("Na:solute_symport",  "SSS",          "Sodium/solute symporter family"),
        "TIGR02119": ("Na:solute_symport",  "panF",         "Sodium/pantothenate symporter PanF"),
        "TIGR02121": ("Na:solute_symport",  "putP",         "Sodium/proline symporter PutP"),
        "TIGR03647": ("Na:solute_symport",  "Na_symp_sm",   "Sodium/substrate symporter, small subunit"),
        "TIGR03648": ("Na:solute_symport",  "Na_symp_lg",   "VC_2705 family sodium/solute symporter"),
        # Other (kept in the library but not sodium-coupled secondary transporters)
        "TIGR00900": ("other",              "DHA3",         "DHA3/2.A.1.21 drug:H+ antiporter (NOT a Na+ transporter)"),
        "TIGR01106": ("other",              "Na_K_ATPase",  "Na,H/K-ATPase alpha subunit (P-type ATPase, primary pump)"),
    }
    ...
    if args.bin_ext != "NA":
        ...
        print("Looks like you did not provide an extension for your genomes/bins or assemblies, so PortNaGenie does "
    ...
    try:
        ...
        answer = input("Would you like PortNaGenie to proceed and potentially overwrite files in this directory? (y/n): ")
        if answer == "y":
                    ...
                    for line in testFile:
                        if re.match(r'>', line):
                            if re.findall(r'\|]', line):
                                ...
                                print("Unfortunately, this is a problem for PortNaGenie because it uses that character "
    ...
    print("Profiling sodium-coupled transporters with TIGRFAM HMMs (hmmsearch --cut_tc)")
    ...
    out.write("cell,ORF,HMM,gene_symbol,description,transport_mode,evalue,bitscore\n")
    for key in HMMdict.keys():
        for j in HMMdict[key]:
            hmm = HMMdict[key][j]["hmm"]
            acc = hmm.split(".")[0]
            meta = HMM_META.get(acc, ("unknown", "unknown", "unknown"))
            mode, gene_sym, desc = meta
            out.write(",".join([
                key, j, hmm, gene_sym, "\"" + desc + "\"", mode,
    ...
    for line in summary:
        ...
        if ls[4].startswith("\"") and not ls[4].endswith("\""):
            # spans multiple split fields -- find the closing quote
            j = 4
            desc_parts = [ls[4]]
        ...
        record = {
            "hmm": hmm, "gene_sym": gene_sym, "desc": desc, "mode": mode,
    ...
    # =================================================================
    # Cluster ORFs by genomic proximity (putative co-localized transporters)
    # =================================================================
            ...
            for k in clusters:
                for l in RemoveDuplicates(k):
                    ...
                    out.write(",".join([
                        ...
                        rec["mode"], str(rec["evalue"]), str(rec["bit"]),
    ...
    # Final summary CSV
    # =================================================================
    out = open("%s/portnagenie-summary.csv" % args.out, "w")
    out.write("file,ORF,gene,gene_symbol,description,transport_mode,evalue,bit_score,cluster_id,seq\n")
    ...
    #     2. Transport-mode summary rows (counts collapsed by category)
    # =================================================================
    print("....")
    print(".....")

    # Build per-HMM row labels: e.g. "TIGR00773_NhaA_Na:H_antiport"
    hmm_rows = []
    for acc in sorted(HMM_META.keys()):
        mode, gene_sym, _ = HMM_META[acc]
        hmm_rows.append("%s_%s_%s" % (acc, gene_sym, mode))

    # Transport mode summary rows
    mode_rows = ["TOTAL_" + m for m in TRANSPORT_MODES]

    cats = hmm_rows + mode_rows

    Dict = defaultdict(lambda: defaultdict(list))
    final = open("%s/portnagenie-summary.csv" % args.out, "r")
    for line in final:
        if re.match(r'#', line):
            continue
        ls = line.rstrip().split(",")
        ...
        # ls[5] is the transport_mode column (after gene, gene_symbol, description)
        # but description was quoted and may have absorbed commas. Trust HMM_META instead.
        meta = HMM_META.get(hmm_acc)
        if not meta:
            continue
        mode, gene_sym, _ = meta

        # Per-HMM row
        hmm_row = "%s_%s_%s" % (hmm_acc, gene_sym, mode)
        Dict[cell][hmm_row].append(hmm_acc)
        # Transport-mode aggregate row
        Dict[cell]["TOTAL_" + mode].append(hmm_acc)
    ...
    outHeat = open("%s/portnagenie.heatmap.csv" % outDirectory, "w")
    outHeat.write("X,")
    ...
    print("Finished!")
    print("")
    print("Results are written to %s/portnagenie-summary.csv and %s/portnagenie.heatmap.csv" % (args.out, args.out))
